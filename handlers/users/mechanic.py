...
async def authorizeUser(mail, password_user, message: types.Message, state: FSMContext):
    ...
    try:
        ...
        school = r_user_json['school']
        await state.update_data(user_school=school)
...
async def menu(message: types.Message, state: FSMContext):
    ...
    button5 = types.InlineKeyboardButton(text="Подбери репититора", url="https://api.sokratapp.ru/api/courses/category/repetitori/")
...
async def mesAdmin(text, state: FSMContext):
    data = await state.get_data()
    email = data.get('user_email')
    await bot.send_message(ADMINS, f"Пользователь {email} {text}")
